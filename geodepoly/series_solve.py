...
def durand_kerner(coeffs, iters=1000, tol=1e-14, restarts=3):
    ...
    for attempt in range(restarts):
        ...
        roots = [
            R * cmath.exp(2j * math.pi * (k + 0.3 * attempt) / n) for k in range(n)
        ]
...
def series_solve_all(
    ...
    try:
        r0 = series_one_root(
            coeffs, center=None, max_order=max_order, boots=boots, tol=tol, refine=True
        )
    except Exception:
        r0 = None
    ...
    polished = [
        newton_refine(coeffs, halley_refine(coeffs, z, steps=8), steps=20, tol=1e-15)
        for z in dk_roots
    ]
    return polished
